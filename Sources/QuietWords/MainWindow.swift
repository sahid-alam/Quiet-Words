        ...
        Form {
            ...
            Section("Clean-up") {
                ...
                Text(fillerWords.joined(separator: ", "))
                    .font(.caption).foregroundStyle(.secondary)
                Toggle("Collapse stutters", isOn: $settings.collapseStutters)
                Text("\"the the build is is failing\" becomes \"the build is failing\". Words that legitimately double — had, that, very — are left alone.")
                    .font(.caption).foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
